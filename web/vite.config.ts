...
// `example_website/index.php` serves its own copy of the login page (str_replace-substituting the same
// _REPLACE_ME_IS_PUBLIC_MODE token) - keep it byte-identical to the mod-served one rather than
// maintaining a second implementation. Login-mode build only.
function copyToExampleWebsite(): Plugin {
    const exampleWebsiteLogin = fileURLToPath(new URL("../example_website/login.html", import.meta.url));
    return {
        name: "ae2-copy-login-to-example-website",
        apply: "build",
        async closeBundle() {
            await copyFile(join(assetsDir, "login.html"), exampleWebsiteLogin);
        },
    };
}
...
export default defineConfig(({ command, mode }) => {
    ...
    return {
        plugins: [
            ...
            command === "build" && isLogin && copyToExampleWebsite(),
        ],
        build: {
            outDir: assetsDir,
            emptyOutDir: false,
